...
if (game_vars): 
	if(save_vars) 
		foreach (_REQUEST as var => value) 
			...
			dbn("update db_name_db_vars set value = 'value' where name = '" .
...
END
	...
	db("select name, min, max, value,descript from db_name_db_vars order by name")
...
if (show_active)) 
	...
	db("select last_request,login_name,login_id,clan_sym,clan_sym_color,clan_id from db_name_users where last_request > ".(time()-300)." && login_id > 1 order by last_request desc")
	player = dbr()
	if(!player)
...
if(ban))
	if(ban == 2) #show ban a player page.
		...
		if(!ban_target || ban_target < 1 || !ban_time)
			db("select login_name,login_id from db_name_users where banned_time <= " .
		 ...
		 elseif (ban_target > 0)
			db("select login_name from db_name_users where login_id = ban_target")
			ban_info = dbr()
			if(ban_time > max_time || ban_time < -1)
			 ...
			 elseif(!sure)
				...
				dbn("update db_name_users set banned_time = 'ban_time', banned_reason = 'ban_reason' where login_id = 'ban_target'")
				if(ban_time > 0)
...
#		print_page("Ban Player",out)
	 elseif(unban))
		db("select login_name from db_name_users where login_id = unban")
		ban_info = dbr()
		insert_history(unban,"Was Un-Banned from the game")
		dbn("update db_name_users set banned_time = '0', banned_reason = '' where login_id = 'unban'")
	...
	db("select login_name, login_id, banned_time, banned_reason from db_name_users where banned_time = -1 || banned_time > ".time()." order by banned_time desc")
...
if(reset_signup)) 
	...
	dbn("update db_name_users set joined_game = " . time())
...
if(reset))
	if(reset == 1)
	 ...
	 elseif(reset == 2) 
		...
		dbn("delete from db_name_users where login_id != " . ADMIN_ID)
		dbn("delete from db_name_user_options where login_id != " . ADMIN_ID)
		out .= "Users & their options deleted.<br>"

		dbn("update db_name_users set turns='1000', turns_run='0', location=1, ship_id=NULL, cash='100000000', on_planet='0', last_attack='0', last_attack_by='', ships_killed='0', ships_lost=0, ships_lost_points=0, ships_killed_points=0, genesis='1', gamma='1', clan_sym='', clan_sym_color='', clan_id=0, fighters_killed='0', one_brob='0', alpha='1', sn_effect='0', last_request='0', score='0', tech='100000' where login_id=" . ADMIN_ID)
		out .= "Admin account refurbished.<br>"

		dbn("delete from db_name_news")
		out .= "News erased.<br>"

		dbn("delete from db_name_planets")
		out .= "Planets erased.<br>"

		dbn("delete from db_name_messages where login_id != " . ADMIN_ID . ' && login_id != ' . OWNER_ID)
		out .= "Messages deleted.<br>"

		dbn("delete from db_name_politics")
		dbn("INSERT INTO db_name_politics VALUES ( '1', 'Monarch', '0', '', '0')")
		dbn("INSERT INTO db_name_politics VALUES ( '2', 'Industry Senator', '0', '', '0')")
		dbn("INSERT INTO db_name_politics VALUES ( '3', 'Military Senator', '0', '', '0')")
		dbn("INSERT INTO db_name_politics VALUES ( '4', 'Defense Senator', '0', '', '0')")
		dbn("INSERT INTO db_name_politics VALUES ( '5', 'Trade Senator', '0', '', '0')")
		dbn("INSERT INTO db_name_politics VALUES ( '6', 'War Senator', '0', '', '0')")
		dbn("INSERT INTO db_name_politics VALUES ( '7', 'Espionage Senator', '0', '', '0')")
		dbn("INSERT INTO db_name_politics VALUES ( '8', 'Research Senator', '0', '', '0')")
		out .= "Politics refurbished.<br>"

		dbn("delete from db_name_diary where login_id != " . ADMIN_ID . ' && login_id != ' . OWNER_ID)
		out .= "Diaries erased.<br>"

		dbn("delete from db_name_ships")
		out .= "Ships deleted.<br>"

		dbn("delete from db_name_clans")
		out .= "Clans deleted.<br>"

		dbn("delete from db_name_bilkos")
...
if(planet_list) || sort_planets))
	...
	if(!empty(sort_planets))
		...
		db("select login_name,planet_name,location,fighters,colon,cash,metal,fuel,elect,organ from db_name_planets where location != 1 && planet_type >= 0 order by 'sort_planets' going")
	 else 
		db("select login_name,planet_name,location,fighters,colon,cash,metal,fuel,elect,organ from db_name_planets where location != 1 && planet_type >= 0 order by login_name asc, fighters desc, planet_name asc")
...
<ul>
	<li><a href="build_universe.php?build_universe=1&ampprocess=1">Generate New Universe</a></li>
