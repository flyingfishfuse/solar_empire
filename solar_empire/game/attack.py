    ...
    if GameVars.quark and UserShip.has_quark:
        if user_info.on_planet != planet_info.planet_num:
            if user_info.turns_left < GameVars.attack_turn:
		        ...
		        print("There is no point in attacking this planet with the Quark Displacer, as there are no fighters left on it to destroy.<p><a href=planet.php?planet_id=PlanetInfo.planet_id]>Land</a> on it, and then claim it to make it yours.")
		...
		if return_game_var(quark_damage) >= asdf:
		    damage = PlanetInfo.fighters]

		#post_news </b> fired a Quark Displacer at <b class=b1>PlanetInfo.planet_name]</b>.");
		if PlanetInfo.fighters < 1 :
			# send message "The <b class=b1>$user_ship[ship_name]</b> fired at your planet <b class=b1>PlanetInfo.planet_name]</b> with a Quark Displacer, doing <b>$sv_damage</b> damage, and completely destroying all the planets defences.");
			#"You have done <b>$sv_damage</b> damage to planet <b class=b1>PlanetInfo.planet_name]</b>, using <b>GameVars.sv_turns</b> turns. <p>You completly destroyed all defences on <b class=b1>PlanetInfo.planet_name]</b>. <br> - <a href=planet.php?planet_id=PlanetInfo.planet_id]>Land</a><br>";
		else:
			# send_message PlanetInfo.login_id "The <b class=b1>$user_ship[ship_name]</b> fired at your planet <b class=b1>PlanetInfo.planet_name]</b> with a Quark Displacer, doing <b>$sv_damage</b> damage. There was no way for your planetary defences to retaliate.")
			#"You have done <b>$sv_damage</b> damage to planet <b class=b1>PlanetInfo.planet_name]</b>, at a cost of <b>GameVars.sv_turns</b> turns";
            asdf = "asdf"
		#fighters_killed = fighters_killed + '$f_killed' where login_id = User.login_id]");
		#fighters_lost = fighters_lost + '$f_killed' where login_id = PlanetInfo.login_id]");


#terra maelstrom
if  ) :

//if User.on_planet] != PlanetInfo.planet_num :
	db("select * from ${db_name}_planets where planet_id = 'PlanetInfo.planet_num'");
	$planet_info = dbr();
	 GameVars.attack_turn = 50;
	$base_percent = 2;

	if User.turns_left <  GameVars.attack_turn :
	...
	elif User.location!= PlanetInfo.location'] :
		print_page("Terra Maelstrom","The planet <b class=b1>PlanetInfo.planet_name]</b> is not in this system.");
	elif User.turns_run< GameVars.turns_before_planet_attack && User.login_id!= ADMIN_ID :
		print_page("Terra Maelstrom","You can't attack during the first <b>GameVars.turns_before_planet_attack</b> turns of having your account.");
	elif GameVars.flag_planet_attack == 0 :
		print_page("Quark Displacer","The admin presentlty has planet attacking disabled.");
	elif PlanetInfo.login_id== 1 :
		print_page("Terra Maelstrom","This is an <b class=b1>Admin</b> planet, and cannot be attacked.");
	elif !PlanetInfo.fighters'] :
		print_page("Terra Maelstrom","There is no point in attacking this planet with the Terra Maelstrom, as there are no fighters left on it to destroy.<p><a href=planet.php?planet_id=PlanetInfo.planet_id]>Land</a> on it, and then claim it to make it yours.");
		...
		if PlanetInfo.fighters> $sq_damage && User.turns_left >  GameVars.attack_turn){

			#work out how many fighters may be killed in one shot (between 65 and 75 percent) as a max.
			$max_fig_kills = round((PlanetInfo.fighters/ 100) * mt_rand(65,75));
	...
	#		$damage_done = round(PlanetInfo.fighters] /100 * ($base_percent + $t_dam_done));
	#		$damage_done += round(mt_rand(-$damage_done * .15,$damage_done * .15));
		...
		if !isset($sure) :
			get_var('Terra Maelstrom','attack.php',"Are you sure you want to use the Terra Maelstrom against the planet <b class=b1>PlanetInfo.planet_name]</b>?<br><br>This will use <b>$turn_cost</b> turns and kill about <b>$damage_done</b> fighters.",'sure','yes');
		}

		charge_turns($turn_cost);
		$out_str ="";

		dbn("update ${db_name}_planets set fighters = fighters - '$sw_damage' where planet_id = PlanetInfo.planet_num");
		PlanetInfo.fighters-= $sw_damage;
		post_news("<b class=b1>User.login_name]</b> fired a Terra Maelstrom at <b class=b1>PlanetInfo.planet_name]</b>.");
		if PlanetInfo.fighters< 1 :
			send_message(PlanetInfo.login_id'],"The <b class=b1>$user_ship[ship_name]</b> fired at your planet <b class=b1>PlanetInfo.planet_name]</b> with a Terra Maelstrom, doing <b>$sw_damage</b> damage, and completely destroying all the planets defences.");
			dbn("update ${db_name}_planets set fighters = 0 where planet_id = PlanetInfo.planet_num");
			$f_killed = PlanetInfo.fighters'];
			PlanetInfo.fighters= 0;
			$out_str .= "You have done <b>$sw_damage</b> damage to planet <b class=b1>PlanetInfo.planet_name]</b>, using <b>$turn_cost</b> turns. <p>You completly destroyed all defences on <b class=b1>PlanetInfo.planet_name]</b>. <br> - <a href=planet.php?planet_id=PlanetInfo.planet_id]>Land</a><br>";

		} else:
			send_message(PlanetInfo.login_id'],"The <b class=b1>$user_ship[ship_name]</b> fired at your planet <b class=b1>PlanetInfo.planet_name]</b> with a Terra Maelstrom, doing <b>$sw_damage damage</b>. There was no way for your planetary defences to retaliate.");
			$f_killed = $sw_damage;
			PlanetInfo.fighters-= $sw_damage;
			$out_str .= "You have done <b>$sw_damage</b> damage to planet <b class=b1>PlanetInfo.planet_name]</b>, at a cost of <b>$turn_cost</b> turns";
		}
		dbn("update ${db_name}_users set fighters_killed = fighters_killed + '$f_killed' where login_id = User.login_id]");
		dbn("update ${db_name}_users set fighters_lost = fighters_lost + '$f_killed' where login_id = PlanetInfo.login_id]");
...
if $target > 0 :
	...
	elif isset($sure) != 'yes' && (ereg("hs",$target_ship['config']) || User.login_id== ADMIN_ID) :
		get_var('Attack','attack.php',"Are you sure you want to attack ".print_name($target)."?",'sure','yes');
	elif !isset($sure) && ereg("hs",$target_ship['config']) :
