...
def build_ship(player):
    """
    player is a USER_ID

    """
    user      = return_user_by_id(player)
    user_ship = return_ship_by_user_id(player)
	...
	if mass : 
		ship_stats =  ship_types[ mass]
	 	take_flag = 1
 	elifuser.game_login_count > 0 : 
	 ship_stats =  ship_types[ ship_type]
	 take_flag = 1


"Admin has set the game up so as that ship is not available for purchase"
...
def Seatogues_spacecraft_emporium():
    #Bulk Purchase of ships
    ship      = "why is this shit so similar yet so fucking DIFFERENT!!!"
    quantity  = "PHP IS FOR MINDLESS PEONS OF A SITH LORD"
    user      = return_user_by_id(player)
    #check to ensure are only able to bulk buy merchants
    ...
    #check to allow user to enter the number of ships they want to buy.		 
	 ...
	 elifship_name) :  #confirm purchase.
		 rs = "<p><a href=earth.php>Back to Earth.</a>"
		 rs + "<br><a href=earth.php?ship_shop=1>Return to Ship Shop</a>"
...
elifgot_a_brob == 1 and eregi("oo", ship_stats['config']) : 
...
elifship_type == 1 ||  ship_type == 0 : 
	 error_str = "You may not buy this ship type."
elifuser['cash'] <  ship_stats['cost'] : 
