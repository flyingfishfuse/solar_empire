		...
		if target_planet.location ==  planet.location:
			...
			if(!annihil):
				...
				if(target_planet['colon  >3000):
					colonists_killed = round(target_planet['colon  /100 * 4)
					colonists_killed += round(rand(-colonists_killed * .15,colonists_killed * .15))
					if(colonists_killed > (target_planet['colon  - (target_planet['alloc_fight  + target_planet['alloc_elect  + target_planet['alloc_organ ))) :
					...
					out + "<br>The missile also killed colonists_killed colonists."

				elif (target_planet[colon] >0) :
					colonists_killed = target_planet[colon]
					fight_killed = target_planet['alloc_fight 
					elect_killed = target_planet['alloc_elect 
					organ_killed = target_planet['alloc_organ 
					out + "<br>The missile also killed all colonists_killed colonists that were on the planet."
				...
				out + "<p>Damage report ends"
				dbn("update :db_name}_planets set fighters = fighters - 'damage_done', colon=colon-'colonists_killed', alloc_fight=alloc_fight-'fight_killed', alloc_elect=alloc_elect-'elect_killed', alloc_organ=alloc_organ-'organ_killed' where planet_id = 'destination'")
...
#quark disrupter.
	if(User.turns_left < $sv_turns) :
		...
		dbn("update $:db_name_planets set fighters = fighters - '$sv_damage' where planet_id = $planet_num");
		...
		if(planet_info.fighters  < 1) :
			...
			dbn("update $:db_name_planets set fighters = 0 where planet_id = $planet_num");
...
def terra_maelstrom_attack(planet_id, attacker_id)
	...
	#		$damage_done += round(mt_rand(-$damage_done * .15,$damage_done * .15));
		...
		dbn("update $:db_name_planets set fighters = fighters - '$sw_damage' where planet_id = $planet_num");
